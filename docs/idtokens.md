# ID Token Zoo

This document contains example ID Tokens from different providers. We truncate the signatures.

## Azure

### ID Token

```json
{
    "payload": {
    "ver": "2.0",
    "iss": "https://login.microsoftonline.com/9188040d-6c67-4c5b-b112-36a304b66dad/v2.0",
    "sub": "AAAAAAAAAAAAAAAAAAAAAJ8PFm0pjpXKQouYRalE11g",
    "aud": "bd345b9c-6902-400d-9e18-45abdf0f698f",
    "exp": 1737500954,
    "iat": 1737414254,
    "nbf": 1737414254,
    "preferred_username": "alice@gmail.com",
    "oid": "00000000-0000-0000-7862-618d09e9fa0e",
    "email": "alice@gmail.com",
    "tid": "9188040d-6c67-4c5b-b112-36a304b66dad",
    "nonce": "pElF-ABr22cAQOTAC0qpxI83OH14Hu7fjRSWzS6ViLY",
    "aio": "DoD*c*IDip3fgOs3T8dIIBWw!JwcIwhQCwMcpNinmjEss4Ifu0PKKMPCiuJOXBAtX8OObt*128kwC7cPM97!AHy8mw1kRA9P5dcw6wlj8doC1j5nn03eNizuiwI9JMgdD1I0rfWBClENOSqDUg4ODsuPds!G1NtVGt6bxfRJrM81"
    },
    "protected": {
    "typ": "JWT",
    "alg": "RS256",
    "kid": "aB0xDdGXk535PvewBP9Hl5pf7wc"
    }
}
```

### Refreshed ID Token

```json
{
"payload": {
    "ver": "2.0",
    "iss": "https://login.microsoftonline.com/9188040d-6c67-4c5b-b112-36a304b66dad/v2.0",
    "sub": "AAAAAAAAAAAAAAAAAAAAAJ8PFm0pjpXKQouYRalE11g",
    "aud": "bd345b9c-6902-400d-9e18-45abdf0f698f",
    "exp": 1737500957,
    "iat": 1737414257,
    "nbf": 1737414257,
    "preferred_username": "alice@gmail.com",
    "oid": "00000000-0000-0000-7862-618d09e9fa0e",
    "email": "alice@gmail.com",
    "tid": "9188040d-6c67-4c5b-b112-36a304b66dad",
    "aio": "DmyH0vHZqJZRcv752O*ph8K!x!EOAIk!e1nSWrtlGxUlMccl2n5qHa9vY4YtAfWma!VI3BqIfsfzYdrrMDW22D5qIRAOYDP9utExgqTitwxNf83p*3nLqtCTnND!GVwhM35BGCaLqsA9MF8gu1dbUfTPgYxD4yTOH3sZq3hDyAbY"
    },
"protected": {
    "typ": "JWT",
    "alg": "RS256",
    "kid": "aB0xDdGXk535PvewBP9Hl5pf7wc"
    }
}
```

### ID Token (Device Flow)

Azure ID Token generated by device flow. Notice the lack of a nonce.

```json
{
    "payload": {
        "ver": "2.0",
        "iss": "https://login.microsoftonline.com/9188040d-6c67-4c5b-b112-36a304b66dad/v2.0",
        "sub": "AAAAAAAAAAAAAAAAAAAAANKeVt8iRZ3WPZXpU7diums",
        "aud": "096ce0a3-5e72-4da8-9c86-12924b294a01",
        "exp": 1765821421,
        "iat": 1765734721,
        "nbf": 1765734721,
        "name": "Alice Example",
        "preferred_username": "alice@gmail.com",
        "oid": "00000000-0000-0000-fec5-70d05b099e0b",
        "email": "alice@gmail.com",
        "tid": "9188040d-6c67-4c5b-b112-36a304b66dad",
        "aio": "DmLIsc45QLCIj8SoQ44heloi2KDEfQyXtnQYyDOA4ZF5EJkF6mTiudq6vJF918jl9BPLCmDsklXHmGo*yLvsbTf3DxD0C5kU82*OL3GNaFDkcrzbwhT2w9ucnqa6L1IZquw2F2CZNlCcjbNbQKhIVs5Eq4KBIeYDkI4yRTES4wCb"
    },
    "protected": {
        "typ": "JWT",
        "alg": "RS256",
        "kid": "fr4hEBEECiE7xC8D3u2vlr3n9ok"
    }
}
```

## Google

### ID Token

```json
{
"payload": {
    "iss": "https://accounts.google.com",
    "azp": "992028499768-ce9juclb3vvckh23r83fjkmvf1lvjq18.apps.googleusercontent.com",
    "aud": "992028499768-ce9juclb3vvckh23r83fjkmvf1lvjq18.apps.googleusercontent.com",
    "sub": "103030642802723203118",
    "email": "alice@gmail.com",
    "email_verified": true,
    "at_hash": "fTQQ5_pA8i-Zx_Bif9PVrA",
    "nonce": "oZifhxF_UkB0AG6K1hPyEsvDCHuqW3QsyK4O_XIlTzU",
    "name": "Alice Example",
    "picture": "https://lh3.googleusercontent.com/a/ACg8ocJie7Hgt4fitN0_GWXaFHYBuy1UMkr_ufkLXTW7MEpA_UMx1XlU=s96-c",
    "given_name": "Alice",
    "family_name": "Example",
    "iat": 1737415178,
    "exp": 1737418778
    },
"protected": {
    "alg": "RS256",
    "kid": "6337be6364f3824008d0e9003f50bb6b43d5a9c6",
    "typ": "JWT"
    }
}
```

### Refreshed ID Token

```json
{
"payload": {
    "iss": "https://accounts.google.com",
    "azp": "992028499768-ce9juclb3vvckh23r83fjkmvf1lvjq18.apps.googleusercontent.com",
    "aud": "992028499768-ce9juclb3vvckh23r83fjkmvf1lvjq18.apps.googleusercontent.com",
    "sub": "103030642802723203118",
    "email": "alice@gmail.com",
    "email_verified": true,
    "at_hash": "kUOiPWtlZegMkNufB3eDaA",
    "name": "Alice Example",
    "picture": "https://lh3.googleusercontent.com/a/ACg8ocJie7Hgt4fitN0_GWXaFHYBuy1UMkr_ufkLXTW7MEpA_UMx1XlU=s96-c",
    "given_name": "Alice",
    "family_name": "Example",
    "iat": 1737415180,
    "exp": 1737418780
    },
"protected": {
    "alg": "RS256",
    "kid": "6337be6364f3824008d0e9003f50bb6b43d5a9c6",
    "typ": "JWT"
    }
}
```

### ID Token (Device flow)

Google ID Token generated by device flow. Notice the lack of a nonce.

```json
{
    "payload": {
        "iss": "https://accounts.google.com",
        "azp": "411517154569-3cbcg4kvba8kvrd3dindpv94t2d9qa70.apps.googleusercontent.com",
        "aud": "411517154569-3cbcg4kvba8kvrd3dindpv94t2d9qa70.apps.googleusercontent.com",
        "sub": "103030642802723203118",
        "email": "eth3rs@gmail.com",
        "email_verified": true,
        "at_hash": "ALE2_FRJ3XXdisEubZfZQA",
        "name": "Alice Example",
        "picture": "https://lh3.googleusercontent.com/a/ACg8ocJie7Hgt4fitN0_GWXaFHYBuy1UMkr_ufkLXTW7MEpA_UMx1XlU=s96-c",
        "given_name": "Ethan",
        "family_name": "Heilman",
        "iat": 1765733385,
        "exp": 1765736985
    },
    "protected": {
                "alg": "RS256",
                "kid": "130fdcefcc8ed7be6bedfa6fc879722040c92b38",
                "typ": "JWT"
    }
}
```

### ID Token (Device flow youtube)

Google ID Token generated by device flow for youtube identity of google identity. Notice the lack of a nonce.

```json
{
    "payload": {
        "iss": "https://accounts.google.com",
        "azp": "411517154569-3cbcg4kvba8kvrd3dindpv94t2d9qa70.apps.googleusercontent.com",
        "aud": "411517154569-3cbcg4kvba8kvrd3dindpv94t2d9qa70.apps.googleusercontent.com",
        "sub": "10842343242342423432422",
        "email": "alice-9000@pages.plusgoogle.com",
        "email_verified": true,
        "at_hash": "qFMmW3nW0h7eHSzAsVkKSw",
        "name": "alice",
        "picture": "https://lh3.googleusercontent.com/a/ACg8ocI-179IZIzAcDGf_fXN99XhrXGe8aB5f8fk_Htn8CYP7CMF9A=s96-c",
        "given_name": "alice",
        "iat": 1765733836,
        "exp": 1765737436
    },
    "protected": {
        "alg": "RS256",
        "kid": "130fdcefcc8ed7be6bedfa6fc879722040c92b38",
        "typ": "JWT"
    }
}
```

## Hello.dev

### ID Token

This was generated using <https://playground.hello.dev/> with gitlab claims enabled

```json
{
"payload": {
    "iss": "https://issuer.hello.coop",
    "aud": "app_HelloDeveloperPlayground_Iq2",
    "nonce": "26fe2dc0-b405-40cb-bfdc-e26aee0564ce",
    "jti": "jti_0FmofYWcyShupLOb9X3YwaIl_EVL",
    "sub": "8dfb4b1a-2b9e-4f59-a2dc-33e6806f3fe0",
    "tenant": "personal",
    "email": "alice@gmail.com",
    "email_verified": true,
    "gitlab": {
        "id": "76205622",
        "username": "Alice"
    },
    "iat": 1743381952,
    "exp": 1743382252
},
"protected": {
    "alg": "RS256",
    "typ": "JWT",
    "kid": "2025-01-15T16:56:49.668Z_735-1c5"
    },
}
```

### Key Bound ID Token

```json
{
  "payload": {
    "iss": "https://issuer.hello.coop",
    "aud": "app_xejobTKEsDNSRd5vofKB2iay_2rN",
    "nonce": "UIjSuNcpCiJ6Vn1SqCaE4ILexfFsolukHmqlgKbC1p0",
    "jti": "jti_U82eEJjPL9pRfLXNcfEMe5SJ_M3w",
    "sub": "sub_NdETpSN2LthxgTKdrBcLK2au_TDg",
    "tenant": "personal",
    "name": "Anonymous Author",
    "picture": "https://lh3.googleusercontent.com/a/ACg8ocIdbWtaAGFsizjWVh7Q6C-XDBuSoUOpf7d7nGqgNQ-9yHmenNA=s96-c",
    "email": "anon.author.aardvark@gmail.com",
    "email_verified": true,
    "iat": 1761943045,
    "exp": 1761943345,
    "cnf": {
        "jwk": {
            "alg": "ES256",
            "crv": "P-256",
            "kty": "EC",
            "x": "6hgrwR47GqR6wpeTUAusxBYbwnO5I_B5nTaO0YH75Uk",
            "y": "H0ZtI1Bbytlvfn3ej3eW0qVkXpyuFSRVmuLtwRq3UyM"
        }
    }
  },
  "signatures": [
    {
      "protected": {
        "alg": "RS256",
        "typ": "dpop+id_token",
        "kid": "2025-09-05T11:09:18.267Z_ab4-956-RS256"
      },
      "signature": "CqXs..."
    }
  ]
}
```

### ID Token (Device Flow)

Hello ID Token generated by device flow. Unlike Azure or Google it has a nonce as so can be used for OpenPubkey.

```json
{
    "payload": {
        "iss": "https://issuer.hello.coop",
        "aud": "app_fzr7iWr50CWQkGDrLCZBYQc4_2Ak",
        "nonce": "3gsrMHXxKocLkp8sfxXLj3NTWs0isLGQECiWDFPIKMw",
        "jti": "jti_KADuW9LC080Vd8a52FaI57hw_y4Q",
        "sub": "sub_zTWxYNTlVzLiN8DaQQrunm6C_kkV",
        "tenant": "personal",
        "name": "Alice Example",
        "picture": "https://pictures.hello.coop/r/530b7cd3-ed42-498e-9250-9e6fd09e0dd8.png",
        "email": "alice@gmail.com",
        "email_verified": true,
        "iat": 1765737153,
        "exp": 1765737453
    },
    "protected": {
        "alg": "RS256",
        "typ": "JWT",
        "kid": "2025-09-05T11:09:18.267Z_ab4-956-RS256"
    }
}
```

## AWS Cognito

### ID Token (with token revocation enabled)

```json
{
    "payload": {
        "at_hash": "-9rmUKO5T6OZrkWR_dnZzQ",
        "sub": "f4f8b4a8-b061-7039-6671-844b2e140c9d",
        "email_verified": true,
        "iss": "https:\/\/cognito-idp.us-east-1.amazonaws.com\/us-east-1_FqOCYEQAc",
        "cognito:username": "f4f8b4a8-b061-7039-6671-844b2e140c9d",
        "nonce": "Ze_mXtKziFWWMbt_7B0IBYIylXg1qVbtyUnwVBkkA1I",
        "origin_jti": "d1c87520-e3a5-489b-90c5-3c86e5bc20d6",
        "aud": "7mutue6chhphanddiel1r4u0qv",
        "token_use": "id",
        "auth_time": 1764973015,
        "exp": 1764976615,
        "iat": 1764973015,
        "jti": "2178848f-99ae-4389-a4e2-155d6c60594a",
        "email": "alice@example.com"
    },
    "signatures": [
        {
            "protected": {
                "kid": "UGIevE1I5DkrQL0VFF7nRif5Z5G5PXaHEArHtBu\/HM0=",
                "alg": "RS256"
            },
            "signature": "Q7kGG..."
        }
    ]
}
```

### ID Token (without token revocation enabled)

```json
{
    "payload": {
        "at_hash": "aeOHnzHGgPHiyI2Kvw4tcQ",
        "sub": "f4f8b4a8-b061-7039-6671-844b2e140c9d",
        "email_verified": true,
        "iss": "https:\/\/cognito-idp.us-east-1.amazonaws.com\/us-east-1_FqOCYEQAc",
        "cognito:username": "f4f8b4a8-b061-7039-6671-844b2e140c9d",
        "nonce": "uhPcEjVMklIg5qrC10ToRfD_KiE6GCsXQYNs8Gi4hao",
        "aud": "1gf0blnk3kkj5njus0fgqj4qd4",
        "event_id": "01007bb1-7b8b-412b-b095-846501c77b2c",
        "token_use": "id",
        "auth_time": 1764972249,
        "exp": 1764975849,
        "iat": 1764972249,
        "email": "alice@example.com"
    },
    "signatures": [
        {
            "protected": {
                "kid": "UGIevE1I5DkrQL0VFF7nRif5Z5G5PXaHEArHtBu\/HM0=",
                "alg": "RS256"
            },
            "signature": "Q7kGG..."
        }
    ]
}
```

## Forgejo Actions

### ID Token

```json
{
    "payload": {
        "actor": "USER",
        "actor_id": "1",
        "aud": "openpubkey-zoo-example",
        "base_ref": "",
        "event_name": "push",
        "exp": 1784585263,
        "head_ref": "",
        "iat": 1784581663,
        "iss": "https://codeberg.org/api/actions",
        "nbf": 1784581663,
        "ref": "refs/heads/main",
        "ref_protected": "false",
        "ref_type": "branch",
        "repository": "USER/REPO",
        "repository_id": "2",
        "repository_owner": "USER",
        "repository_owner_id": "1",
        "run_attempt": "1",
        "run_id": "2043",
        "run_number": "2",
        "sha": "0000000000000000000000000000000000000000",
        "sub": "repo:USER-1/REPO-2:ref:refs/heads/main",
        "workflow": "test.yaml",
        "workflow_ref": "USER/REPO/.forgejo/workflows/test.yaml@refs/heads/main"
    },
    "protected": {
        "alg": "RS256",
        "kid": "yQCTwdl9pjrj9ineITmxuAv4RAaLlQVXcu5KB39q7GU",
        "typ": "JWT"
    }
}
```

## Authentik


### Key Bound ID Token

```
{
    "payload": {
        "iss": "http://localhost:9000/application/o/openpubkey-test/",
        "sub": "2ba228f8875aa6cc3ab0eb7fc54ff9c682ccc147bc12ca022b910ad023a51595",
        "aud": "4MpVsa2M0CJ5owpFnf1L1FesjCTaK7tVm2U36RmF",
        "exp": 1784587746,
        "iat": 1784587446,
        "auth_time": 1784585012,
        "acr": "goauthentik.io/providers/oauth2/default",
        "amr": [
            "pwd"
        ],
        "nonce": "IyLaURkg66v3n7U-VaEwm5bBytZkY_PfUBJOvrW2agk",
        "sid": "70c9c8aa4347060e4d5b2828c5242b7e9ab436439a48287210da1ed8ec2a605c",
        "jti": "QzwMq51CcssIi8JahgLzC9LoE5hiqYTB4NVVvboI",
        "cnf": {
            "jwk": {
            "crv": "P-256",
            "kty": "EC",
            "x": "61u0boQIL-o-qhcx4caPCFx7PNleccaXmb6Ybhvo3k0",
            "y": "8sZd6CUQdn4v3H741MkZ-ubX4sfLP8j2HbwxZ3rK5Ng"
            }
        },
        "email": "alice@example.com",
        "email_verified": false,
        "name": "authentik Default Admin",
        "given_name": "authentik Default Admin",
        "preferred_username": "alice",
        "nickname": "alice",
        "groups": [
            "authentik Admins"
        ]
},
    "signatures": [
        {
            "protected": {
                "alg": "RS256",
                "kid": "SGay0U5Grp4HYXbySuZ1aOQETBW00l5YUryzvGpHAv1gfXo_0IfEOg-hHLcegzLhdRtM_q4asnO5XFpRdItmOg",
                "typ": "dpop+id_token"
            },
            "signature": "FudX7X..."
        }
    ]
}
```

### Key Bound ID Token (Refreshed)

The refreshed ID Token is identical to the original token. Perhaps Authentik returns the original ID Token until it expires.

```
{
    "payload": {
        "iss": "http://localhost:9000/application/o/openpubkey-test/",
        "sub": "2ba228f8875aa6cc3ab0eb7fc54ff9c682ccc147bc12ca022b910ad023a51595",
        "aud": "4MpVsa2M0CJ5owpFnf1L1FesjCTaK7tVm2U36RmF",
        "exp": 1784587746,
        "iat": 1784587446,
        "auth_time": 1784585012,
        "acr": "goauthentik.io/providers/oauth2/default",
        "amr": [
            "pwd"
        ],
        "nonce": "IyLaURkg66v3n7U-VaEwm5bBytZkY_PfUBJOvrW2agk",
        "sid": "70c9c8aa4347060e4d5b2828c5242b7e9ab436439a48287210da1ed8ec2a605c",
        "jti": "QzwMq51CcssIi8JahgLzC9LoE5hiqYTB4NVVvboI",
        "cnf": {
            "jwk": {
            "crv": "P-256",
            "kty": "EC",
            "x": "61u0boQIL-o-qhcx4caPCFx7PNleccaXmb6Ybhvo3k0",
            "y": "8sZd6CUQdn4v3H741MkZ-ubX4sfLP8j2HbwxZ3rK5Ng"
            }
        },
        "email": "alice@example.com",
        "email_verified": false,
        "name": "authentik Default Admin",
        "given_name": "authentik Default Admin",
        "preferred_username": "alice",
        "nickname": "alice",
        "groups": [
            "authentik Admins"
        ]
},
    "signatures": [
        {
            "protected": {
                "alg": "RS256",
                "kid": "SGay0U5Grp4HYXbySuZ1aOQETBW00l5YUryzvGpHAv1gfXo_0IfEOg-hHLcegzLhdRtM_q4asnO5XFpRdItmOg",
                "typ": "dpop+id_token"
            },
            "signature": "FudX7X..."
        }
    ]
}
```
